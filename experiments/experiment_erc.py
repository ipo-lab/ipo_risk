...
import torch
from ts.cov import GarchDCCNetNNL
from ts.loss import loss_erc_cov
from ts.popt import RPCCC, RPCCCOLS, RPDCC, RPDCCOLS
from experiments.utils import load_response, load_features, get_train_dates, plot_z_rp_cov, get_trade_weights, \
...
# --- CCC:
plot_z_rp_cov(w_ols=w_ccc_ols.values, w_ipo=w_ccc_ipo.values, cov_mat=cov_mat)
plt.savefig(out_dir + 'rp_ccc_' + feature + '.png')
# --- DCC:
plot_z_rp_cov(w_ols=w_dcc_ols.values, w_ipo=w_dcc_ipo.values, cov_mat=cov_mat)
...
plt.legend(["OLS", "IPO"])
plt.savefig(out_dir + 'rp_ccc_' + feature + '_equity.png')
