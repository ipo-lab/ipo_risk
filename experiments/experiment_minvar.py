...
import torch
from ts.loss import loss_minvar_cov
from experiments.utils import load_response, load_features, get_train_dates,  plot_z_minvar_cov, get_trade_weights, \
     compute_returns, optimize_model
from ts.popt import MinVarCCC, MinVarCCCOLS, MinVarDCC, MinVarDCCOLS, GarchDCCNetNNL
...
for i in range(len(oos_dates)):
    ...
    model_ccc = optimize_model(x=x_is, y=cov_mat_is, model_init=MinVarCCC, loss_fn=loss_minvar_cov,
                               lr=lr, n_epochs=n_epochs, n_starts=n_starts, n_ahead=n_ahead)
    # --- Train IPO Models: DCC
    model_dcc = optimize_model(x=x_is, y=cov_mat_is, model_init=MinVarDCC, loss_fn=loss_minvar_cov,
                               lr=lr, n_epochs=n_epochs, n_starts=n_starts, n_ahead=n_ahead)
