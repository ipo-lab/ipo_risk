...
from experiments.utils import load_response, load_features, get_train_dates,  plot_z_maxdiv, get_trade_weights, compute_returns, optimize_model
...
for i in range(len(oos_dates)):
    ...
    model_ccc = optimize_model(x=x_is, y=y_is, model_init=MaxDivCCC, loss_fn=loss_maxdiv,
                               lr=lr, n_epochs=n_epochs, n_starts=n_starts, n_ahead=n_ahead, init='random')
    # --- Train IPO Models: DCC
    model_dcc = optimize_model(x=x_is, y=y_is, model_init=MaxDivDCC, loss_fn=loss_maxdiv,
                               lr=lr, n_epochs=n_epochs, n_starts=n_starts, n_ahead=n_ahead, init='random')
